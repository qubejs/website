---
title: Glossary
description: Definition of various terms in the context of the Qubejs Framework
---
...
The ability to describe a feature in a modular manner is what makes _Qubejs_
unique.

One super-power of a Feature is that it is _transferable_ across any
Qubejs-enabled Apps. This means you can literally drop a feature into an unrelated
Qubejs App and have it work with minimal or no configuration changes. This allows
Qubejs to build a **family of Apps** that share a bunch of features!

<Aside>
 **Mini App**

Features are also referred as **Mini Apps** in the Qubejs's parlance. When a
...
These are synonymous with Features. Qubejs encourages dividing your application
...
<Aside>
  A Plugin in the context of Qubejs is just a pluggable API and should not be
  confused with
...
its typed-attributes. Schemas are pervasively used in Qubejs to define various
...
CMS itself is a plugin inside Qubejs and accessible via `vyuh.content`. It is used
to describe dynamic content with a known schema. Qubejs takes special advantage of
...
Qubejs has several built in actions such as Navigation, Script execution,
Restarting, Changing themes, etc.

You can add your own actions and make them relevant to your app.

> Like everything else in the _Qubejs Framework_, Action has counterparts on CMS
...
eventing system inside Qubejs acts like an event-bus where different features and

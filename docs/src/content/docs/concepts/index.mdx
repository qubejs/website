...
### Presenting the Qubejs framework

> Qubejs is a framework to build _modular_, _scalable_ **Flutter** apps driven by
...
We meet in the middle with the **Qubejs Framework**, where the CMS components come
...
- Features are **transferable** and can be moved between a family of
  Qubejs-enabled apps. This works well for larger companies with a host of apps.
...
The Qubejs Framework bridges the gap of a No-Code/Low-Code tool by separating the

...
The `QubejsContentWidget` is a powerful, standalone Flutter widget that simplifies
...
It follows a simple binding-based architecture (via the `QubejsContentBinding`)
...
![QubejsContentWidget Architecture](./vyuh-content-widget-arch.png)

<Aside title={'Gradual Adoption from a Widget to the full Framework'} type={'tip'}>
The `QubejsContentWidget` offers a gradual approach to integrating CMS-driven 
content into existing Flutter applications. It provides several advantages for 
apps that aren't ready to fully migrate to the Qubejs framework:
...
5. **Future-Proofing**: As you become more familiar with CMS-driven development,
   you'll be better prepared for a potential full migration to the Qubejs
   framework in the future.

By using the `QubejsContentWidget`, you can experience the benefits of
...
content-blocks. This enhances the `QubejsContentWidget` to work with a wider
variety of content.
...
### 2. Initialize QubejsContentBinding
...
The simplest way to use `QubejsContentWidget` is with a `Document`. A Document
(`vyuh.document`) is a built-in content type that provides a standard structure
for content in Qubejs applications. This is part of the
...
```dart
QubejsContentWidget.fromDocument(
    ...
    return Column(
      children: [
        ...
        if (document.items != null && document.items!.isNotEmpty)
          Expanded(
            child: QubejsContentBinding.content.buildContent(
              context,
...
```dart
QubejsContentWidget<Conference>(
...
The `QubejsContentWidget` is a powerful widget for rendering content from a CMS.

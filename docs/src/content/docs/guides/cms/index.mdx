...
The CMS (Content for short) is a flagship extension provided by the Qubejs
...
The Qubejs Framework supports extensions that allow you to extend the capabilities
...
Sanity. Here is the typical `sanity.config.ts` file for a Qubejs project:
...
help in rendering the Qubejs App with the previously mentioned features from the
CMS setup:
...
`plugins` and bootstraps the Qubejs App to life.

Continue exploring the various aspects of Qubejs for a flexible system of

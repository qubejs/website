...
description:
  Fetching content from a third-party API and rendering it inside Qubejs
---

import { Aside } from '@astrojs/starlight/components'

Rendering content from an API remains one of the most popular use cases of the
Qubejs Framework. This is expected as most real-world apps will fetch data from an
...
recommended way of invoking APIs in Qubejs. The plugin wraps the `HttpClient` from
...
Qubejs has rich provision to integrate custom API endpoints and render their

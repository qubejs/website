...
_Condition_ is one of the core elements that is built into the Qubejs Framework.
...
framework like Qubejs. It allows controlling the runtime behavior from a CMS
...
because **conditions are a first class citizen of the Qubejs Framework**.
...
Conditions in Qubejs evaluate to one of many pre-determined **String** values.
...
The Qubejs Framework allows you to define conditions that can be used to control

---
title: Qubejs Structure Plugin
description:
  Learn how to organize and combine Sanity schemas from multiple features using
  the Qubejs structure plugin
---

import { Aside } from '@astrojs/starlight/components'

The Qubejs structure plugin (`@vyuh/sanity-plugin-structure`) is a powerful tool
...
When building a Qubejs application, each feature defines its schemas and exports
...
> The Qubejs structure plugin reads the `FeatureDescriptor` instances and
...
![Qubejs Structure Plugin](images/vyuh-structure-plugin.png)
...
- `@vyuh/sanity-plugin-structure`: The Qubejs structure plugin that combines and
...
also building a Qubejs App, these can be leveraged on the **Flutter** side. 

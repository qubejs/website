...
title: Events
description:
  Events are a way to get notified of important lifecycle events in the Qubejs
...
final feature = FeatureDescriptor(
  name: 'root',
  title: 'Qubejs Root Feature',
  description: 'The root feature for the Qubejs Demo app',
  init: () async {
...
method emits a type-safe event that is a subclass of `QubejsEvent`. Here is a
...
```dart
final class SystemReadyEvent extends QubejsEvent<void> {
...
The `QubejsEvent` class is the base class for all events in the Qubejs Framework. It
...
Currently, there is only one standard event in the Qubejs Framework:
...
You can create custom events by extending the `QubejsEvent` class. Here is an
example of a custom event that carries a payload of type `String`:

```dart
class CustomEvent extends QubejsEvent<String> {
...
The event system in the Qubejs Framework is a global event system. The event
...
Events are a powerful way to communicate between features in the Qubejs Framework.

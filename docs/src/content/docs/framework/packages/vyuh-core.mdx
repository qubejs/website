---
title: Qubejs Core
description: The main runtime package that provides the default platform and bootstrapping capability for Qubejs
---
...
The Qubejs Core package provides the foundation for building Qubejs applications. It includes the core runtime, plugin system, and extension mechanisms that power the framework.

## Core Components

### runApp()
The main entry point for bootstrapping a Qubejs application. It initializes the platform, sets up the plugin system, and starts the application with the specified features. The function handles the initialization sequence, ensuring plugins are loaded in the correct order and features are properly registered before the application starts.
...
#### FeatureDescriptor
A comprehensive descriptor for defining features in a Qubejs application. It encapsulates all aspects of a feature including its metadata, initialization logic, routing, and extensions. Features are the primary building blocks of a Qubejs application, providing modular and reusable functionality that can be composed together. The descriptor ensures features are properly isolated and can be independently developed and tested.
...
#### Navigation Plugin
Powers Qubejs's routing and navigation system. Provides a declarative way to define routes, handle deep linking, and manage navigation state. Includes both the plugin interface and a default implementation using GoRouter.
...
abstract class EventPlugin extends Plugin {
  // Subscribe to events
  DisposeFunction on<T extends QubejsEvent>(QubejsEventListener<T> listener);
  
  // One-time event subscription
  void once<T extends QubejsEvent>(QubejsEventListener<T> listener);
  
  // Emit events
  void emit<T extends QubejsEvent>(T event);
}
```
...
Qubejs provides default implementations for most plugins:
...
Qubejs provides a set of customizable platform widgets for common UI patterns:
...
typedef AppBuilder = Widget Function(QubejsPlatform platform);

...
Now that we know how to setup a feature in Qubejs, let's see how we can integrate
...
we will set up the studio and use the Qubejs Framework to build a simple blog
...
A faster way to scaffold a Qubejs project would be with the <PubBadge
name={'vyuh_cli'} inline /> tool. The Qubejs CLI can generate a complete Qubejs
Project, a Flutter feature package and an NPM schema package as well.

Refer to the [Qubejs CLI](/intro/cli) guide for more details.

</Aside>

### **An Aside on developing with a CMS 🤔**

Qubejs is designed to be CMS-agnostic and allows a different style of building
...
These dependencies will help in the schema development with _Sanity_ and _Qubejs_.
...
For more details on the Qubejs Structure plugin, refer to
...
![Blog post rendered with Qubejs](images/simulator.png)
...
One of the most powerful features of Qubejs is the ability to see content changes
...
Behind the scenes, Qubejs leverages Sanity's Live Content API to establish a
...
In addition to live updates, Qubejs also provides a way to manually refresh routes
...
on the Device Simulator. With the power of the Qubejs Framework, you can make
...
can do with custom content and the Qubejs Framework. We will explore more in other
guides.
